...
import { useNFC } from "@/src/components/hooks/useNFC";
import NFCLinkingModal from "@/src/components/modals/NFCLinkingModal";
import RewardModal from "@/src/components/modals/RewardModal";
...
import NFCDeviceCard from "@/src/components/molecules/NFCDeviceCard";
...
export default function ChildrenWalletsScreen() {
  ...
  const findChildIndex = () => {
    if (selectedChildId) {
      const index = childrenWallets.findIndex(
  ...
  const [parentBalance, setParentBalance] = useState(500.0);
  ...
  const handleLoadMoney = () => {
    ...
    console.log(
      `€${amount.toFixed(2)} ont été ajoutés au wallet de ${selectedChild.name}`
    );
  };
  ...
  const handleUnloadMoney = () => {
    ...
    console.log(
      `€${amount.toFixed(2)} ont été retirés du wallet de ${selectedChild.name}`
    );
  };
  ...
  const handleGiveReward = (amount: number, reason: string, type: string) => {
    ...
    setChildrenWallets(updatedChildren);
    console.log(
      `Récompense de €${amount.toFixed(2)} donnée à ${
        selectedChild.name
      } pour: ${reason}`
    );
  };
  ...
  const handleNFCLink = (nfcId: string, deviceType: string) => {
    ...
    if (selectedChild) {
      const newDevice = {
        id: nfcId,
        type: deviceType,
        name: `${deviceType === "bracelet" ? "Bracelet" : "Tag"} ${
          selectedChild.name
        }`,
  ...
  const removeNFCDevice = (childId: number, deviceId: string) => {
    setLinkedNFCDevices((prev) => ({
      ...prev,
      [childId]:
        prev[childId]?.filter((device) => device.id !== deviceId) || [],
    }));
  };
  ...
  const renderChildCard = (child) => (
    <View key={child.id} style={styles.carouselCard}>
      ...
      >
        <LinearGradient
          colors={
            child.isActive ? ["#4F46E5", "#7C3AED"] : ["#9CA3AF", "#6B7280"]
          }
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
        ...
        >
          ...
          <Text style={styles.childBalance}>€{child.balance.toFixed(2)}</Text>
          <View style={styles.limitProgress}>
            <View style={styles.limitInfo}>
              <Text style={styles.limitText}>
                €{child.weeklySpent.toFixed(2)} / €
                {child.weeklyLimit.toFixed(2)} cette semaine
              </Text>
            </View>
    ...
    return (
      <View style={styles.childDetails}>
          ...
          {linkedNFCDevices[selectedChild.id]?.length > 0 ? (
            linkedNFCDevices[selectedChild.id].map((device) => (
              <NFCDeviceCard
                ...
                device={device}
                onRemove={(deviceId) =>
                  removeNFCDevice(selectedChild.id, deviceId)
                }
                onToggleStatus={(deviceId) =>
                  toggleNFCDeviceStatus(selectedChild.id, deviceId)
                }
                onSettings={(deviceId) =>
        ...
        <View style={styles.categoriesSection}>
          ...
          {Object.entries(selectedChild.categories).map(
            ([category, amount]) => (
            ...
            >
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      transaction.type === "income" ? "#DCFCE7" : "#FEF2F2",
                  },
                ]}
              >
  ...
  return (
    <SafeAreaView style={styles.container}>
            ...
            {childrenWallets.map((_, index) => (
              <View
                ...
                style={[
                  ...
                  {
                    backgroundColor:
                      selectedChildIndex === index ? "#4F46E5" : "#E5E7EB",
                  },
                ]}
              />
            ))}
      ...
      {/* Load Money Modal */}
      <Modal visible={showLoadModal} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>
      ...
      {/* Unload Money Modal */}
      <Modal visible={showUnloadModal} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>
              Retirer de l'argent du wallet de{" "}
              {childrenWallets[selectedChildIndex]?.name}
            </Text>
