...
export const useWallets = () => {
  ...
  const topUpMutation = useMutation({
    ...
    onSuccess: (response, variables) => {
      if (response.success && response.data) {
        ...
        Alert.alert('Succès', `€${variables.amount.toFixed(2)} ajoutés avec succès`);
        queryClient.invalidateQueries({ queryKey: ['wallets'] });
        queryClient.invalidateQueries({ queryKey: ['transactions'] });
      }
    },
    onError: (error: any) => {
      Alert.alert('Erreur', error.response?.data?.message || 'Erreur lors du rechargement');
    },
  });
  ...
  const setPrimaryMutation = useMutation({
    ...
    onError: (error: any) => {
      Alert.alert('Erreur', error.response?.data?.message || 'Erreur lors de la définition');
    },
  });
    ...
    onSuccess: (response, walletId) => {
      if (response.success) {
        ...
        if (wallet) {
          ...
          Alert.alert('Succès', `Portefeuille ${!wallet.isActive ? 'activé' : 'désactivé'}`);
        }
      }
    },
    onError: (error: any) => {
  ...
  // Helper functions
  const createWallet = useCallback((data: any) => {
    return createWalletMutation.mutateAsync(data);
  }, [createWalletMutation]);

  const topUpWallet = useCallback((data: TopUpWalletRequest) => {
    return topUpMutation.mutateAsync(data);
  }, [topUpMutation]);

  const updateWalletData = useCallback((walletId: number, data: any) => {
    return updateWalletMutation.mutateAsync({ walletId, data });
  }, [updateWalletMutation]);

  const deleteWallet = useCallback((walletId: number) => {
    return deleteWalletMutation.mutateAsync(walletId);
  }, [deleteWalletMutation]);

  const setPrimaryWallet = useCallback((walletId: number) => {
    return setPrimaryMutation.mutateAsync(walletId);
  }, [setPrimaryMutation]);

  const toggleWalletStatus = useCallback((walletId: number) => {
