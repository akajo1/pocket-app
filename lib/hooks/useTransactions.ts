...
export const useTransactions = (filters: TransactionFilters = {}) => {
  ...
  const transactionsQuery = useQuery({
    ...
    queryFn: async () => {
      const response = await transactionApi.getTransactions(mergedFilters);
  ...
  const analyticsQuery = useQuery({
    queryKey: ['transactions', 'analytics', mergedFilters.startDate, mergedFilters.endDate],
    queryFn: async () => {
      const period = mergedFilters.startDate && mergedFilters.endDate 
        ? undefined 
        : '30';
  ...
  const exportTransactionsMutation = useMutation({
    mutationFn: ({ format, filters }: { format: 'csv' | 'pdf'; filters: TransactionFilters }) =>
      transactionApi.exportTransactions(format, filters),
    onSuccess: () => {
      Alert.alert('Succès', 'Export généré avec succès');
    },
    onError: (error: any) => {
      Alert.alert('Erreur', error.response?.data?.message || 'Erreur lors de l\'export');
    },
  });

  // Helper functions
  const createTransaction = useCallback((data: CreateTransactionRequest) => {
    return createTransactionMutation.mutateAsync(data);
  }, [createTransactionMutation]);

  const updateTransactionData = useCallback((transactionId: number, data: any) => {
  ...
  const exportTransactions = useCallback((format: 'csv' | 'pdf' = 'csv', exportFilters: TransactionFilters = {}) => {
    return exportTransactionsMutation.mutateAsync({ format, filters: exportFilters });
  ...
  const refreshAnalytics = useCallback(() => {
    return queryClient.invalidateQueries({ queryKey: ['transactions', 'analytics'] });
  }, [queryClient]);
  ...
  return {
    ...
    isLoading: isLoading || transactionsQuery.isLoading || analyticsQuery.isLoading,
    error: error || transactionsQuery.error?.message || analyticsQuery.error?.message,
