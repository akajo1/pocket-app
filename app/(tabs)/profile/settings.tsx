...
import { Globe, Moon, Palette, Database, Download, Trash2, Info, CircleHelp as HelpCircle, MessageSquare, Star } from 'lucide-react-native';
...
export default function SettingsScreen() {
  ...
  const dataActions = [
    ...
    },
    {
      ...
      action: () => Alert.alert('Attention', 'Cette action est irréversible. Êtes-vous sûr ?'),
      color: '#DC2626'
    }
  ];
  ...
  const supportActions = [
    ...
    },
    {
      ...
      action: () => Alert.alert('Support', 'Formulaire de contact à implémenter')
    },
    {
      icon: Star,
    ...
    },
    {
      ...
      action: () => Alert.alert('À propos', 'MyWallet v1.0.0\n\nDéveloppé avec ❤️')
    }
  ];
    ...
    <View key={index} style={styles.settingItem}>
      <TouchableOpacity 
        ...
        <View style={styles.settingLeft}>
          <View style={[
            styles.settingIcon,
            { backgroundColor: item.enabled !== undefined ? 
              (item.enabled ? '#EEF2FF' : '#F3F4F6') : '#F3F4F6' }
          ]}>
            <item.icon 
              size={20} 
              color={item.color || (item.enabled !== undefined ? 
                (item.enabled ? '#4F46E5' : '#9CA3AF') : '#6B7280')} 
            />
          </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.appInfoCard}>
          <View style={styles.appIcon}>
            <Text style={styles.appIconText}>MW</Text>
          </View>
